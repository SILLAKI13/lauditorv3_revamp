...
class NoSSLv3SocketFactory : SSLSocketFactory {
    ...
    constructor(sourceUrl: URL) {
        this.delegate = NetCipher.getHttpsURLConnection(sourceUrl).sslSocketFactory
    }
    ...
    private fun makeSocketSafe(socket: Socket): Socket {
        var safeSocket = socket
        if (safeSocket is SSLSocket) {
            safeSocket = NoSSLv3SSLSocket(safeSocket)
        }
        return safeSocket
    }
    ...
    private inner class NoSSLv3SSLSocket(delegate: SSLSocket) : DelegateSSLSocket(delegate) {
        override fun setEnabledProtocols(protocols: Array<String>?) {
            ...
            if (updatedProtocols != null && updatedProtocols.size == 1 && "SSLv3" == updatedProtocols[0]) {
                val enabledProtocols = ArrayList(Arrays.asList(*delegate.enabledProtocols))
    ...
    open inner class DelegateSSLSocket(protected val delegate: SSLSocket) : SSLSocket() {
        override fun getSupportedCipherSuites(): Array<String> {
            return delegate.supportedCipherSuites
        }
